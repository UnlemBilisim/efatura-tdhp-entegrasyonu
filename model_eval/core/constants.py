"""Sabitler: yol/host varsayilanlari, UBL namespace'leri, TDHP hesap sozlugu
ve system prompt. Diger tum core modulleri buradan import eder."""
...
DEFAULT_SECTOR = (
    "NACE 24.10.10 - Pik demir ve manganezli dokme demir (aynali demir/spiegeleisen) "
    "uretimi (kulce, blok veya diger birincil formlarda); Ana demir ve celik urunleri "
    "ile ferro alasimlarin imalati (NACE 24.10) ana grubunun alt kodu"
)
...
# NOT: Asagidaki listede kasitli olarak hicbir 3 haneli TDHP kodu ORNEK OLARAK
# verilmiyor. Eskiden "(orn. 100, 120, 191, 320, 391, 600, 770, 689)" gibi bir
# ornek listesi vardi; bu, modele rastgele bir onyargi asiliyordu (770 ornekte
# vardi, 730/760 yoktu - model 730 yerine surekli 770'i "halusinasyon" yapiyordu).
# "Hesap plani vermeden test" ilkesini korumak icin hicbir kod ornegi verilmez.
SYSTEM_PROMPT = """Sen deneyimli bir Serbest Muhasebeci Mali Musavir (SMMM) yapay zeka asistanisin.
Gorevin, sana verilen bir faturanin muhasebe kaydini Turkiye'de resmi olarak kullanilan
Tek Duzen Hesap Plani'na (TDHP) gore, tamamen kendi bilgine dayanarak olusturmak.

Kurallar:
- Sana hesap plani listesi VERILMEYECEK. Dogru hesap kodlarini kendi bilgin ile belirle.
- SADECE TDHP'nin 3 haneli ana hesap kodunu kullan. Sirkete ozel alt/muavin hesap
  numarasi (orn. 191.01.00020) UYDURMA, sadece 3 haneyi yaz.
- Kayit karsilikli (double-entry) olmali: toplam Borc tutari toplam Alacak tutarina esit olmali.
- Kesme/yorum/aciklama YAZMA. Cevabini SADECE su JSON semasina uygun ver, baska hicbir metin ekleme:

{"entries": [{"account_code": "3 haneli kod", "amount": sayi, "dc": "Borc" veya "Alacak"}]}
"""

# --with-glossary modu icin: bu, orijinal "hesap plani vermeden" deneyinin
# TERSI bir senaryodur (modele referans veriliyor). Ayri bir arac/karsilastirma
# kolu olarak kullanilmasi icin var - varsayilan calistirmada DEVREYE GIRMEZ.
# Kaynak: tekduzenhesapkoralari.txt (resmi TDHP, tum 271 hesap kodu - sadece bu
# veri setinde gecen ~26 kod degil, TDHP'nin tamami).
TDHP_GLOSSARY = {
    "100": "Kasa",
    "101": "Alınan Çekler",
    "102": "Bankalar",
